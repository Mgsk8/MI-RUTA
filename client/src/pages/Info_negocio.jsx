...
export default function InfoNegocio() {
    ...
    // Función para obtener datos del negocio y reseñas
    const fetchData = async () => {
        try {
            ...
            if (userReviewData.data?.length > 0) {
                const userReview = userReviewData.data[0];
            ...
            // Agregar nombres de usuarios a las reseñas
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            setShowAlert(true); // Mostrar aviso
            setTimeout(() => setShowAlert(false), 3000); // Ocultar aviso después de 3 segundos
    ...
    const handleDeleteReview = async () => {
        try {
            const id_usuario = localStorage.getItem(LOCAL_STORAGE_TERMS.ID_LOGGED_USER);
            await deleteReview({ id_usuario }, id_lugar);
            setUserReview(null);
            setReview("");
            setCalificacion(0);
